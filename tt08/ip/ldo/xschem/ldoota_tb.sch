...
if MC_RUNS = 0
	...
	.include ldoota_ac_ol_ops.inc

	set nobreak
	set width=300
	print col olgain_1hz, pm, ugbw, f1_3db, f2_3db > ldoota_ac_ol.csv
else
	...
	set nobreak
	set width=300
	print olgain_1hz, pm, ugbw, f1_3db, f2_3db > ldoota_ac_ol.csv
end
goto done
...
if MC_RUNS = 0
	...
	.include ldoota_ac_cmrr_psrr_ops.inc

	set nobreak
	set width=300
	print col cmrr_10Hz, cmrr_10MHz, psrr_p_10Hz, psrr_p_10MHz, psrr_n_10Hz, psrr_n_10MHz > ldoota_ac_cmrr_psrr.csv
else
	...
	set nobreak
	set width=300
	print cmrr_10Hz, cmrr_10MHz, psrr_p_10Hz, psrr_p_10MHz, psrr_n_10Hz, psrr_n_10MHz > ldoota_ac_cmrr_psrr.csv
end
goto done
...
if MC_RUNS = 0
	...
	.include ldoota_tran_outrng_ops.inc

	set nobreak
	set width=300
	print col neg_out_comp, pos_out_comp > ldoota_tran_outrng.csv
else
	...
	set nobreak
	set width=300
	print neg_out_comp, pos_out_comp > ldoota_tran_outrng.csv
end
goto done
...
if MC_RUNS = 0
	...
	.include ldoota_tran_cminp_ops.inc

	set nobreak
	set width=300
	print col pos_cm_bound, neg_cm_bound > ldoota_tran_inprng.csv
else
	...
	set nobreak
	set width=300
	print pos_cm_bound, neg_cm_bound > ldoota_tran_inprng.csv
end
goto done
...
if MC_RUNS = 0
	...
	.include ldoota_tran_slew_ops.inc

	set nobreak
	set width=300
	print col inp_offset, slew_pos, slew_neg > ldoota_tran_slew.csv
else
	...
	set nobreak
	set width=300
	print inp_offset, slew_pos, slew_neg > ldoota_tran_slew.csv
end
goto done
...
setplot noise1
write ldoota_noise1_opout.raw all
...
set width=300
set noprintscale
print col f0, n_f0, f1, n_f1 > ldoota_ac_inoise.csv
...
if MC_RUNS = 0
	...
	.include ldoota_tran_thd_ops.inc

	set nobreak
	set width=300
	print col thd > ldoota_tran_thd.csv
else
...
.endc
" }
C {devices/lab_pin.sym} 790 -460 0 0 {name=p3 lab=VCM}
C {devices/lab_pin.sym} 940 -590 0 1 {name=p4 lab=OPOUT}
...
C {devices/lab_pin.sym} 520 -180 0 0 {name=p14 lab=VCM}
C {devices/capa.sym} 810 -530 0 0 {name=COUT
m=1
value=1f
footprint=1206
...
C {devices/lab_pin.sym} 520 -120 0 0 {name=p17 lab=IB}
...
C {devices/res.sym} 900 -530 2 0 {name=ROUT
value=100G
footprint=1206
device=resistor
m=1}
C {devices/vsource.sym} 310 -250 0 0 {name=VSUPP value="DC 3 AC 0"}
C {devices/res.sym} 610 -740 3 0 {name=RFB
value=100G
footprint=1206
device=resistor
m=1}
...
C {devices/vsource.sym} 310 -150 0 0 {name=VSUPN value="DC 3 AC 0"}
C {devices/lab_pin.sym} 405 -200 0 1 {name=p10 lab=0
}
C {ldoota.sym} 610 -590 0 0 {name=X1}
C {devices/vcvs.sym} 810 -700 0 0 {name=E2 value=1}
C {devices/lab_pin.sym} 810 -670 0 0 {name=p9 lab=VSS}
C {devices/lab_pin.sym} 770 -680 0 0 {name=p11 lab=VSS}
